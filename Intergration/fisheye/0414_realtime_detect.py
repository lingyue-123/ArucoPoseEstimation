...
JSON_FILE_NAME = "/data/yrq/take_gun_start.json"   # 全局变量：按需修改 JSON 文件名

# 全局变量：重叠面积比例阈值 (例如 0.3 表示检测框有 30% 以上的面积在防区内才算入侵)
INTRUSION_THRESHOLD = 0.1

# -------------------------------
# Car 进入 ROI 事件防抖参数
# -------------------------------
ENTER_CONFIRM_FRAMES = 2   # 连续 2 帧检测到，确认进入
EXIT_CONFIRM_FRAMES = 40    # 连续 8 帧未检测到，确认离开（防止偶尔漏检）
...
# def append_one_to_json(json_file):
#     """
#     每次触发事件时，向 JSON 文件中的 events 列表追加一个 1
#     """
#     try:
#         if not os.path.exists(json_file):
#             data = {"events": []}
...
#         data["events"].append(1)

#         with open(json_file, "w", encoding="utf-8") as f:
#             json.dump(data, f, ensure_ascii=False, indent=4)

#         print(f"[JSON] 写入 1 成功 -> {json_file}")
#     except Exception as e:
#         print(f"[JSON] 写入失败: {e}")

def append_one_to_json(json_file):
    """
    每次触发事件时，直接覆盖整个 JSON 文件，写入 {"events": [1]}
    """
    try:
        # 直接构造你要覆盖写入的数据
        data = 1
    ...
    # 'w' 模式打开文件，会自动清空原文件并写入新内容
        ...
        print(f"[JSON] 完全覆盖写入 1 成功 -> {json_file}")
    except Exception as e:
        print(f"[JSON] 写入失败: {e}")
...
def is_in_roi(xyxy, roi_polygon):
    """
    用检测框与 ROI 的重叠面积比例判断是否进入
    xyxy: [x1, y1, x2, y2]
    roi_polygon: numpy array 格式的 OpenCV 多边形坐标，例如 shape 为 (N, 1, 2) 或 (N, 2)
    返回:
        intrusion: bool (是否入侵)
        overlap_ratio: float (当前的重叠比例，方便调试打印)
    """
    x1, y1, x2, y2 = map(int, xyxy)
    ...
    # 2. 构造 ROI 的多边形
    # OpenCV 的多边形通常包含多余的维度 (N, 1, 2)，需要 reshape 成 (N, 2)
    roi_coords = roi_polygon.reshape(-1, 2).tolist()
    roi_poly = Polygon(roi_coords)
    
    # 可选：如果 ROI 多边形存在自交等不规则情况，buffer(0) 可以自动修复它，防止报错
    if not roi_poly.is_valid:
        roi_poly = roi_poly.buffer(0)

    # 3. 计算交集面积与检测框自身面积
    # intersection() 方法计算两个多边形的相交部分
    intersection_area = bbox_poly.intersection(roi_poly).area
    bbox_area = bbox_poly.area
    
    # 防御性编程：防止除以 0 (虽然正常的检测框面积不会为 0)
    if bbox_area == 0:
        return False, 0.0
    ...
    # 5. 根据全局阈值判断是否入侵
    is_intrusion = overlap_ratio >= INTRUSION_THRESHOLD
    
    return is_intrusion, overlap_ratio
...
def run(
    ...
    # 初始化 JSON 文件
    # init_json_file(JSON_FILE_NAME)

    # 上一帧是否有 Car 在 ROI 中
    # prev_car_in_roi = False

    car_present_state = False  # 当前稳定状态：是否认为“车在 ROI 内”
    car_enter_count = 0        # 连续检测到车在 ROI 内的帧数
    car_miss_count = 0         # 连续未检测到车在 ROI 内的帧数
    ...
    try:
        while True:
            ...
            # 车辆进入 ROI：仅在 "无车 -> 有车" 的瞬间写一次 1
            # if (not prev_car_in_roi) and current_car_in_roi:
            #     append_one_to_json(JSON_FILE_NAME)

            # prev_car_in_roi = current_car_in_roi

            # -----------------------------------------
            # Car 进入/离开 ROI 的防抖状态机
            # -----------------------------------------
            if current_car_in_roi:
                car_enter_count += 1
                car_miss_count = 0
            else:
                car_miss_count += 1
            ...
            # 从“无车” -> “有车”：需要连续若干帧确认，避免偶发误检
            if (not car_present_state) and (car_enter_count >= ENTER_CONFIRM_FRAMES):
                car_present_state = True
                append_one_to_json(JSON_FILE_NAME)
                print(f"[EVENT] Car confirmed ENTER ROI, json written -> {JSON_FILE_NAME}")
                break

            # 从“有车” -> “无车”：需要连续若干帧都没检测到，才认为真的离开
            elif car_present_state and (car_miss_count >= EXIT_CONFIRM_FRAMES):
                car_present_state = False
                print("[EVENT] Car confirmed LEAVE ROI")
