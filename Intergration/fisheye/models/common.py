...
class DCNv2(nn.Module):
    """
    Deployment fallback version of DCNv2.
    Keeps the same constructor/interface, but replaces deform_conv2d
    with standard conv2d so ONNX can be exported.
    """
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=1, dilation=1, groups=1, deformable_groups=1):
        super(DCNv2, self).__init__()
        ...
        # keep parameter names compatible with old checkpoints as much as possible
        self.weight = nn.Parameter(
            torch.empty(out_channels, in_channels // groups, *self.kernel_size)
        )
        self.bias = nn.Parameter(torch.empty(out_channels))

        # keep this branch so old state_dict can still load with fewer surprises
        out_channels_offset_mask = (
    ...
    def forward(self, x):
        # keep the offset branch for checkpoint compatibility,
        # but ignore its output in deployment/export mode
        _ = self.conv_offset_mask(x)
