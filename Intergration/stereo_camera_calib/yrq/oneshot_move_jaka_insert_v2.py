...
工作流：
    ...
    10. 按 'q' 退出/home/nvidia/Downloads
...
def main():
    ...
    try:
        take_gun_start = 0
        f_first_pose = [0, 0, 0, 0, 0, 0]
        while True:

            with open('./cover_pose.json', 'r', encoding='utf-8') as f:
                fcntl.flock(f, fcntl.LOCK_SH)
                f_first_pose = json.load(f)
                fcntl.flock(f, fcntl.LOCK_UN)

            with open('./take_gun_start.json', 'r', encoding='utf-8') as f:
                fcntl.flock(f, fcntl.LOCK_SH)
                take_gun_start = json.load(f)
                fcntl.flock(f, fcntl.LOCK_UN)
            ...
            if key in (ord('q'), 27):
                ...
                if args.no_robot:
                    ...
                    ensure_coord_sys(robot, BASE_COORD_SYS, label=f"{BASE_COORD_SYS}(恢复基准坐标系)")
            elif (key == ord('f') and f_first_pose != [0, 0, 0, 0, 0, 0]):
                if moving:
                    logger.warning("运动中，请等待完成")
                    continue
                ...
                if args.no_robot:
                    ...
                    if not ensure_coord_sys(robot, BASE_COORD_SYS, label=f"{BASE_COORD_SYS}(回f点位坐标系)"):
                        continue
                    moving = True
                    ok = execute_move(robot, f_first_pose, move_speed=DEFAULT_SPEED)
                    moving = False
                    if ok:
                        new_tcp = robot.get_tcp_pose()
                        if new_tcp is not None:
                        ...
                        try:
                            ...
                            continue
            elif (key == ord('g') or take_gun_start == 1):
            # elif (key == ord('g')):
                take_gun_start = 0
                try:
                    with open('./take_gun_start.json', 'w', encoding='utf-8') as f:
                        fcntl.flock(f, fcntl.LOCK_EX)
                        json.dump(0, f)
                        f.flush()
                        os.fsync(f.fileno())
