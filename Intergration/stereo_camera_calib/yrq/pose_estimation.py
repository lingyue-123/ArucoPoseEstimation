...
from run_onnx import YOLOv5, filter_box
from pix_2_base_new import get_3D_pose
import argparse
import json
import fcntl
import os
...
def args_parse():
    ...
    parser.add_argument("--l_offset", type = float, default=100.0, help="水平补偿")
    parser.add_argument("--h_offset", type = float, default=0.0, help="高度补偿")
    parser.add_argument("--d_offset", type = float, default=-550.0, help="深度补偿")
    parser.add_argument("--euler_offset", type = float, default=90.0, help="角度补偿")
    parser.add_argument("--scale", type = float, default=0.3, help="复选框缩放比例")
...
def get_depth(depth_img, x, y):
    ...
    return depth_img[y, x]  # 注意: numpy索引为 [行, 列]
...
def main():
    args = args_parse()
    # 模型实例化
    onnx_path = "./cover_depth_best.onnx"
    ...
    try:
        while True:
            ...
            with open('./pose_start.json', 'r', encoding='utf-8') as f:
                ...
                if (color_frame is not None) and (depth_frame is not None):
                    ...
                    if outbox.shape[0] == 1:
                        ...
                        if (counts == 11):
                            counts = 0
                            pose_start = 0
                            with open('./cover_pose.json', 'w', encoding='utf-8') as f:
