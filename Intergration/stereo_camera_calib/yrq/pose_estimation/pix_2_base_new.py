...
def get_3D_pose(pts_pixel, depths, l_offset, h_offset, d_offset, euler_offset):
    ...
    # 相机到基座的外参
    R_cam2base = np.array([[ -0.999224,  -0.033533,   0.020644]
                           [ -0.018906,  -0.051335,  -0.998503]
                           [  0.034543,  -0.998118,   0.050661]])

    t_cam2base = np.array([[ 12.49],
                           [ -205.40],
                           [-152.03]])
...
def get_3D_pose_rpy(pts_pixel, depths):
    # 相机内参
    K = np.array([[610.031, 0,       642.691], 
                  [0,       609.985, 369.464], 
                  [0,       0,       1]])
    # 畸变系数
    ...
    # 相机外参
    R_cam2base = np.array([ [-0.91064418, -0.41313640, -0.00674576],
                            [0.41300970, -0.90963805, -0.04451520],
                            [-0.01225465, -0.04332357, 0.99898593]])
    
    t_cam2base = np.array([[-41.43631272],
                           [-128.38532094],
                           [208.93631078]])

    # 步骤1：将像素坐标转换为法兰坐标系下的3D点
    pts_base = []
    for (u, v), depth in zip(pts_pixel, depths):
        pt_cam = cam_from_pix(u, v, depth, K, dist_coeffs)
        pt_base = R_cam2base @ pt_cam + t_cam2base
        pts_base.append(pt_base.flatten())  # 展平为 (3,)
    pts_base = np.array(pts_base)  # 得到形状 (N, 3)

    # 步骤2：拟合平面并计算法兰坐标系下的6D位姿
    R_plane2cam, t_plane2cam, normal_cam, centroid_cam = fit_plane_and_pose(pts_base)

    # 步骤3：转换到基坐标系
    # R_plane2base, t_plane2base = transform_pose_to_base(R_plane2cam, t_plane2cam, R_cam2base, t_cam2base)

    # 步骤4：转换为欧拉角
    euler_xyz = Rot.from_matrix(R_plane2cam).as_euler("xyz", degrees=True)

    return euler_xyz


def get_3D_pose_one(x, y, depth):
    # 相机内参
    K = np.array([[610.031, 0,       642.691], 
                  [0,       609.985, 369.464], 
                  [0,       0,       1]])
    ...
    # 相机到基座的外参（2025-04-19 标定结果）
    # 来源: HD_0323/data/handeye_eye_to_hand/base_from_cam_result.txt
    # 注意：base_from_cam 需要取逆得到 cam2base
    # R_cam2base = np.array([ [-0.91064418, -0.41313640, -0.00674576],
    #                         [0.41300970, -0.90963805, -0.04451520],
    #                         [-0.01225465, -0.04332357, 0.99898593]])
    ...
    R_cam2base = np.array([ [-0.90880110, -0.41679990, -0.01893174],
                            [0.41719540, -0.90720929, -0.05403053],
                            [0.00534487, -0.05700124, 0.99835980]])
    
    t_cam2base = np.array([[-27.07926168],
                        #    [-92.05947301],
                           [-62.05947301],
                           [206.85590419]])
    
    # 步骤1：将像素坐标转换为相机坐标系下的3D点
    pt_cam = cam_from_pix(x, y, depth, K, dist_coeffs)
    # print(f'相机坐标系下坐标：')
    # print(pt_cam)

    pos_base = R_cam2base @ pt_cam + t_cam2base
    return pos_base