...
# -*- coding: utf-8 -*-
"""
ArUco 光照鲁棒性实验 | ArUco Lighting Robustness Test
...
用法 (Usage):
    ...
    python scripts/aruco_lighting_robustness.py --camera mecheye --marker-ids 0,1
    python scripts/aruco_lighting_robustness.py --camera mecheye --raw
...
def parse_args(argv=None):
    ...
    parser.add_argument('--lighting-robust', action='store_true',
                        help=f'启用光照鲁棒线程：根据参考 ROI 亮度实时调整相机增益（死区±{LR_DEADBAND}，仅调增益不动曝光）')
...
def main():
    ...
    if args.lighting_robust:
        def _lighting_robust_loop():
            while not stop_event.is_set():
                ...
                try:
                    ...
                    new_gain = cur_gain
                    if avg_dev < 0:
                        new_gain = min(cur_gain + step, GAIN_LIMIT_DB)
                    else:
                        new_gain = max(cur_gain - step, 0.0)
                    if abs(new_gain - cur_gain) > 0.01:
        ...
        lr_thread.start()
        logger.info("Lighting robustness thread started (gain-only, deadband=%d)", LR_DEADBAND)
