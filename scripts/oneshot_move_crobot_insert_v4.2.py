...
_BIN_DIR = os.path.join(_ROOT, 'scripts', 'bin')
if _BIN_DIR not in sys.path:
    sys.path.insert(0, _BIN_DIR)
from robot_forcecontrolV3 import RobotController
...
def main():
    ...
    _force_so = os.path.join(_BIN_DIR, 'librobotcontrol.so')
            ...
            if key in (ord('q'), 27):
                ...
                if args.no_robot:
                    ...
                    if (current_marker_type == "插枪") and (already_return_gun == False):
                        ...
                        offset_pose = [92.854, -244.705, -43.728, -3.743, -2.832, -1.42]
                ...
                # old_cwd = os.getcwd()
                # os.chdir(_BIN_DIR)
                # cmd = "sudo ./scripts/simple_test eth2"
                # os.system(cmd)
                ...
                force_ctrl.run_ForceControl_get_the_gun()
                ...
                # robot.move_linear(CartesianPose(*[357.099, 168.745, 197.292, 106.922, 18.805, 94.955]))
                robot.move_joint([112.128, 86.673, -59.18, 144.792, -3.059, -17.912], speed=25)
                ...
                if cover_3D_pose is not None:
                    ...
                    cover_offset_pose = [77.095, 18.253, -297.781, 3.657, -2.192, -1.198]
                    ...
                    #             *target_pose)
                    # logger.info(f"  Target TCP for coarse alignment: []",
                    #             *target_pose)
                    
                    # cur_joint = robot.get_joint_pose()
                    # target_joint = robot.inverse_kinematics(target_pose, cur_joint)
                    # robot.move_joint(target_joint, speed=25)
                ...
                current_matrix = pose_to_matrix(current_pose)
                offset_pose = [60.948, -206.795, 296.431, -14.125, -0.417, 0.467]
                ...
                if args.no_robot:
                    ...
                    if ok:
                        ...
                        current_tool_pose = get_robot_tcp_pose_mm(robot, robot_cfg)
                        if current_tool_pose is not None:
                            advance_pose = offset_pose_along_tool_axis(current_tool_pose, 0.0, axis='z')
                            logger.info("Take gun: advance 120 mm along tool Z-axis")
                            logger.info("  Target: X=%.2f Y=%.2f Z=%.2f Rx=%.2f Ry=%.2f Rz=%.2f",
                                        *advance_pose)
                            robot.set_speed(30)
                ...
                current_matrix = pose_to_matrix(current_pose)
                offset_pose_cover = [42.456, -148.433, 262.275, -0.003, -0.002, -0.0]
