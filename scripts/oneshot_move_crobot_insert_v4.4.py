            ...
            if key in (ord('q'), 27):
                ...
                if args.hw_ae:
                    try:
                        ...
                        logger.debug("Hardware AE disabled at start of move")
                    except Exception:
                        pass
                ...
                # 锁曝光到录模板时的值，在窄带内微调（防止 PnP bias 漂移）
                ...
                if ref_exp is not None and not args.no_robot:
                    ...
                    try:
                        ...
                        logger.info("Exp locked to ref: %.0fus %.1fdB ref_bri=%.0f",
                                    ref_exp_us, ref_gain_db, ref_bri)
                        time.sleep(0.3)
                        cur_gain = ref_gain_db
                        cur_exp = ref_exp_us
                        ...
                        if cur_bri is not None:
                            err = cur_bri - ref_bri
                            logger.info("Exposure check: ref_bri=%.0f cur_bri=%.0f err=%.1f", ref_bri, cur_bri, err)

                            if abs(err) <= 25:
                                logger.info("Bri within deadband, using ref exposure as-is")
                            elif err < 0:  # 太暗 → 提增益 → 提曝光(限 1.5×)
                                for _ in range(5):
                                    if cur_gain < _AE_GAIN_LIMIT_DB:
                                        cur_gain = min(cur_gain + 0.5, _AE_GAIN_LIMIT_DB)
                                        camera.set_gain(cur_gain)
                                        time.sleep(0.1)
                                    elif cur_exp < ref_exp_us * 1.5:
                                        cur_exp = min(cur_exp * 1.15, ref_exp_us * 1.5)
                                    ...
                                    for mid, d in cur_result.items():
                                        if mid in (insert_marker_id, takegun_marker_id):
                                            cur_bri = _marker_roi_brightness(cur_frame, d)
                                            break
                                    if cur_bri is not None and cur_bri > 30:
                                        break
                                if cur_bri is not None and cur_bri <= 30:
                                    logger.warning("Marker too dark after adjustment: bri=%.0f", cur_bri)
                            else:  # 太亮 → 降曝光(限 0.67×) → 降增益
                                for _ in range(5):
                                    if cur_exp > ref_exp_us * 0.67:
                                        cur_exp = max(cur_exp * 0.85, ref_exp_us * 0.67)
                                        camera.set_exposure_time(cur_exp)
                                        time.sleep(0.3)
                                    elif cur_gain > 0:
                                        cur_gain = max(cur_gain - 0.5, 0.0)
                                        camera.set_gain(cur_gain)
                                        time.sleep(0.1)
                                    else:
                                        break
                                    ...
                                    for mid, d in cur_result.items():
                                        if mid in (insert_marker_id, takegun_marker_id):
                                            cur_bri = _marker_roi_brightness(cur_frame, d)
                                            break
                                    if cur_bri is not None and abs(cur_bri - ref_bri) <= 25:
                                        break

                        # 用调整后曝光下的 PnP 重算目标位姿
                        with det_lock:
                            aruco_result = latest_det
                        current_marker_id_m = insert_marker_id if insert_marker_id in aruco_result else (
                            takegun_marker_id if takegun_marker_id in aruco_result else None)
                        if current_marker_id_m is not None and T_g2b_cur is not None:
                            T_aruco2cam_cur = aruco_to_matrix(aruco_result[current_marker_id_m])
                            T_g2b_target = compute_new_tool_pose(
                                base_from_tool_old=T_g2b_cur,
                                cam_from_target_old=T_aruco2cam_cur,
                                tool_from_cam=T_c2g,
                            ...
                            final_pose = matrix_to_pose(T_g2b_target)
                            trans_err, rot_err = compute_pose_error(T_g2b_cur, T_g2b_target)
                            logger.info("Target with adjusted exp: X=%.2f Y=%.2f Z=%.2f",
                                        *final_pose[:3])
                    except Exception:
                        logger.warning("Exposure adjustment failed", exc_info=True)
                    ...
                    if ok:
                        ...
                        if new_tcp is not None:
                            ...
                            logger.info("  Actual TCP: X=%.2f Y=%.2f Z=%.2f Rx=%.2f Ry=%.2f Rz=%.2f",
                                        *new_tcp)
                    if (current_marker_type == "插枪") and (already_return_gun == False):
                        current_tcp = robot.get_tcp_pose()
                        # target_pose = apply_relative_pose(current_tcp, charging_offset)
