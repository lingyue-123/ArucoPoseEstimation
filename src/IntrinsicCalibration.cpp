...
#include <sys/stat.h>
#include <sys/types.h>
#include <opencv2/imgproc.hpp>  // 新增：确保drawChessboardCorners可用
                                     ...
                                     const int &corner_height) 
{
    ...
    corner_image_path_ = img_dir_path_ + "conerdetect/";  // 保留你的拼写错误，不修改
    ...
    // 新增：创建cornerdetect文件夹（匹配你的路径拼写）
    if (opendir(corner_image_path_.c_str()) == nullptr){
        char command[1024];
        sprintf(command, "mkdir -p %s", corner_image_path_.c_str());
        system(command);
        printf("Create dir: %s\n", corner_image_path_.c_str());       
    }
    ...
    for (int i = 0; i < file_names.size(); i++){
        ...
        if (whether_found){
            ...
            if (image_selector.addImage(image_corners)){
                ...
                // 小修复：TermCriteria重复EPS，改为EPS+MAX_ITER（不影响逻辑，仅保证角点细化生效）
                ...
                // ========== 新增：绘制棋盘格角点并保存图像 ==========
                // 1. 灰度图转彩色图（drawChessboardCorners在彩色图上绘制更清晰）
                cv::Mat corner_draw_img;
                cv::cvtColor(input_image, corner_draw_img, cv::COLOR_GRAY2BGR);
                // 2. 绘制检测到的角点（绿色圆圈+棋盘格连线）
                cv::drawChessboardCorners(corner_draw_img, corner_size_, 
                                        image_corners, whether_found);
                // 3. 保存绘制后的角点图像到corner_image_path_路径
                std::string corner_img_path = corner_image_path_ + file_names[i];
                cv::imwrite(corner_img_path, corner_draw_img);
                std::cout << "Save corner detect image: " << corner_img_path << std::endl;
                // ========== 新增结束 ==========

                // display corner detect
                cv::imshow(file_names[i], input_image);  // 原有显示逻辑不变
                cv::waitKey(50);
            }
